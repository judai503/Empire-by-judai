...
function loadBotConfig(conn) {
    try {
        ...
        console.log('Error fatal al cargar configuración del subbot:', e)
        conn.botName = global.namebot || 'Bot';
        conn.botBanner = global.banner || 'https://raw.githubusercontent.com/AdonixServices/Files/main/1754310580366-xco6p1-1754310544013-6cc3a6.jpg';
    }
}
...
global.dfail = (type, m, conn) => {
    const bot = conn.botName || global.botname || 'Bot'
    const pick = (arr) => arr[Math.floor(Math.random() * arr.length)]

    const msg = {
        rowner: pick([
            `🚫 *Acceso restringido*\n\n> Este comando solo lo pueden usar los *creadores del bot*.\n> 🤖 Bot: *${bot}*`,
            `⚠️ Oops… este comando es exclusivo para los *dueños del bot*.\n\n> 🔒 ${bot} protege sus funciones especiales.`,
            `👑 Solo los *creadores del bot* pueden usar *${comando}*.\n\n> 🤖 ${bot} dice: “no es personal, son las reglas 😅”`
        ]),

        premium: pick([
            `💎 Este comando es solo para *usuarios premium*.\n\n> ✨ Activa premium para desbloquear más funciones en *${bot}*.`,
            `🔐 Función premium detectada.\n\n> 🧩 Consigue premium para usar *${comando}* en ${bot}.`,
            `🚀 Este comando está reservado para *usuarios premium*.\n\n> 🤖 ${bot} te espera del lado pro 😉`
        ]),

        group: pick([
            `👥 Este comando solo funciona en *grupos*.\n\n> 📌 Úsalo dentro de un grupo para que ${bot} pueda ejecutarlo.`,
            `⚠️ Comando exclusivo para grupos.\n\n> 🤖 ${bot} necesita un grupo para hacer su magia.`,
            `🔒 *${comando}* solo se puede usar en grupos.\n\n> 🫂 Invítame a uno y lo probamos.`
        ]),

        admin: pick([
            `🛡️ Solo los *administradores del grupo* pueden usar este comando.\n\n> 👑 ${bot} respeta la jerarquía.`,
            `⚠️ Permisos insuficientes.\n\n> 🔐 Necesitas ser *admin del grupo* para usar *${comando}*.`,
            `🚫 Acceso denegado.\n\n> 👮‍♂️ ${bot} solo obedece a los admins aquí.`
        ]),

        botAdmin: pick([
            `🤖 Necesito ser *administrador del grupo* para ejecutar este comando.\n\n> ⚙️ Dame admin y lo hago sin problema.`,
            `⚠️ No tengo permisos de admin.\n\n> 🛠️ Hazme admin para poder usar *${comando}*.`,
            `🚫 ${bot} no es admin en este grupo.\n\n> 👑 Dame permisos y continuamos.`
        ])
    }[type]
